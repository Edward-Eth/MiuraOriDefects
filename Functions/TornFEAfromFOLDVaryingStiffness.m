function [] = TornFEAfromFOLDVaryingStiffness(filename,meshsize,displacement,m,n,Tears,stiffnessvary,rundocfolder)
%TornFEAfromFOLDVaryingStiffness Function generates an abaqus input file from a .FOLD file,
%with preset loading conditions and with a proportion of randomly selected
%folds torn and with foldline stiffnesses varying randomly according to an
%input variance level. Requires the communications toolbox.
...
for ii = 1:length(faces.vertices)
    ...
    for kk = 1:4
        ...
        % there are no repeats and randomly tear a percentage of joins
        if (join_locs(ii,kk,1) > ii)  &&  randsrc(1,1,[1 0;(1-Tears) Tears])
...
%                     'MarkerFaceColor',[0 .75 .75])
            ...
            % Conclude the join if not skipped
