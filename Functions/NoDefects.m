function [filename,width] = NoDefects(a,b,alphaangle,theta,m,n,method,deviation,repeat_number,repeats,tears,stiffnessvary,rundocfolder)
%No Defects .FOLD Generation Function for perfect meshes
%   Creates a .FOLD file without any defects, using the minimum neccessary
%   input arguments
...
%% Define unit cell parameters [Schenk & Guest (2013)] See image in
...
if(stiffnessvary==0)
    if(deviation==0)
        if(tears==0)
            if(repeats==1)
                filename = sprintf('n%i_m%i_a%i_b%i.FOLD',n,m,a,b);
            else
                filename = sprintf('n%i_m%i_a%i_b%i_repeat%i.FOLD',n,m,a,b,repeat_number);
            end
        else
            if(repeats==1)
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_tears%g.FOLD',n,m,a,b,method,tears);
            else
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_tears%g_repeat%i.FOLD',n,m,a,b,method,tears,repeat_number);
            end
        end
    else
        if(tears==0)
            if(repeats==1)
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_dev%g.FOLD',n,m,a,b,method,deviation);
            else
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_dev%g_repeat%i.FOLD',n,m,a,b,method,deviation,repeat_number);
            end
        else
            if(repeats==1)
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_dev%g_tears%g.FOLD',n,m,a,b,method,deviation,tears);
            else
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_dev%g_tears%g_repeat%i.FOLD',n,m,a,b,method,deviation,tears,repeat_number);
            end
        end
    end
else
    if(deviation==0)
        if(tears==0)
            if(repeats==1)
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_stiff%g.FOLD',n,m,a,b,method,stiffnessvary);
            else
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_stiff%g_repeat%i.FOLD',n,m,a,b,method,stiffnessvary,repeat_number);
            end
        else
            if(repeats==1)
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_tears%g_stiff%g.FOLD',n,m,a,b,method,tears,stiffnessvary);
            else
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_tears%g_stiff%g_repeat%i.FOLD',n,m,a,b,method,tears,stiffnessvary,repeat_number);
            end
        end
    else
        if(tears==0)
            if(repeats==1)
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_dev%g_stiff%g.FOLD',n,m,a,b,method,deviation,stiffnessvary);
            else
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_dev%g_stiff%g_repeat%i.FOLD',n,m,a,b,method,deviation,stiffnessvary,repeat_number);
            end
        else
            if(repeats==1)
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_dev%g_tears%g_stiff%g.FOLD',n,m,a,b,method,deviation,tears,stiffnessvary);
            else
                filename = sprintf('n%i_m%i_a%i_b%i_method%i_dev%g_tears%g_stiff%g_repeat%i.FOLD',n,m,a,b,method,deviation,tears,stiffnessvary,repeat_number);
            end
        end
    end
end
